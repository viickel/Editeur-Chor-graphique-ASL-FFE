# -*- coding: utf-8 -*-
"""
Dialogue d'édition des combattants.

Équivalent de FormEditCombattants.vb dans le projet VB.NET original.
Permet d'ajouter, modifier et supprimer des combattants dans le projet.
"""
...
from PyQt6.QtWidgets import (
    ...
    QDialogButtonBox, QMessageBox, QLabel, QSizePolicy
)
...
class DialogCombattants(QDialog):
    """
    Dialogue modal pour gérer la liste des combattants du projet.
    ...
    def __init__(self, liste_combattants: List[Combattant], parent=None):
        super().__init__(parent)

        # On travaille sur une copie profonde pour pouvoir annuler
        self._liste: List[Combattant] = copy.deepcopy(liste_combattants)
        # Combattant actuellement sélectionné pour édition
        self._selected: Combattant | None = None

        self.setWindowTitle("Édition des Combattants")
    ...
    def _build_ui(self) -> None:
        ...
        # --- Liste des combattants (gauche) ---
        left_layout = QVBoxLayout()
        left_layout.addWidget(QLabel("Combattants du projet :"))
        ...
        details_group = QGroupBox("Détails du combattant")
        form = QFormLayout(details_group)
        form.setSpacing(8)

        self.edit_id = QLineEdit()
        self.edit_id.setReadOnly(True)
        self.edit_id.setStyleSheet("background-color: #eeeeee;")
        form.addRow("ID (auto) :", self.edit_id)
        ...
        self.chk_capitaine = QCheckBox("Désigner comme capitaine")
        form.addRow("Capitaine :", self.chk_capitaine)
    ...
    def _refresh_list(self) -> None:
        ...
        for c in self._liste:
            self.list_widget.addItem(str(c))
    ...
    def _clear_form(self) -> None:
        ...
        self.chk_capitaine.setChecked(False)
        self._selected = None
        self.btn_update.setEnabled(False)
        self.btn_delete.setEnabled(False)
        self.edit_nom.setFocus()

    def _display_combattant(self, combattant: Combattant) -> None:
        """Affiche les détails d'un combattant dans le formulaire."""
        self.edit_id.setText(str(combattant.id))
        self.edit_nom.setText(combattant.nom)
        self.edit_prenom.setText(combattant.prenom)
        self.edit_licence.setText(combattant.numero_licence)
        self.chk_capitaine.setChecked(combattant.capitaine)
        self._selected = combattant
    ...
    def _on_selection_changed(self, row: int) -> None:
        """Affiche les détails du combattant sélectionné dans la liste."""
        if row < 0 or row >= len(self._liste):
            self._clear_form()
            return
        self._display_combattant(self._liste[row])

    @pyqtSlot()
    def _on_add(self) -> None:
        """Ajoute un nouveau combattant à la liste."""
        if not self._validate_form():
            return

        # Générer un nouvel ID (max existant + 1, ou 1 si liste vide)
        new_id = max((c.id for c in self._liste), default=0) + 1
        ...
        nouveau = Combattant(
            ...
            capitaine=self.chk_capitaine.isChecked(),
        )
    ...
    def _on_update(self) -> None:
        """Met à jour le combattant sélectionné avec les données du formulaire."""
        if self._selected is None:
            QMessageBox.information(self, "Attention", "Veuillez sélectionner un combattant.")
            return
        if not self._validate_form():
            return
        ...
        self._selected.capitaine = self.chk_capitaine.isChecked()
    ...
    def _on_delete(self) -> None:
        """Supprime le combattant sélectionné après confirmation."""
        if self._selected is None:
            QMessageBox.information(self, "Attention", "Veuillez sélectionner un combattant.")
            return
    ...
    def get_liste(self) -> List[Combattant]:
        """Retourne la liste des combattants éditée (après validation)."""
        return self._liste
